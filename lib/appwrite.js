...
export const sendPasswordResetEmail = async (email) => {
  try {
    await account.createRecovery(
      email,
      'https://cloud.appwrite.io/recovery'  // Update with your actual reset URL
    );
  } catch (error) {
...
export const createUser = async (name, email, password, confirmPassword, account_number, phone, originalReceipt) =>{
try {
  ...
  const newUser = await databases.createDocument(
    ...
    ID.unique(),
    {
      ...
      officialReceipt: originalReceipt
    }
  );
...
export const createLog = async (consumer_id, name, time_stamp, action, email, role) =>{
  ...
  //followUp update
  export const updateComplaintStatus = async (complaintId,  followUp) => {
    try {
  ...
  //withdraw 
  export const updateComplaintStatusToWithdrawn = async (complaintId,  status, reason) => {
    try {
      const complaintStatus = await databases.updateDocument(
        appwriteConfig.databaseId,
        appwriteConfig.complaintsCollectionId,
