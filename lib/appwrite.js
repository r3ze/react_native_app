...
export const appwriteConfig={
    ...
    counterCollectionId: '670de5470029a766eab9', 
...
export const createComplaint = async (form) =>{
  try {
    ...
    const counterDoc = await databases.getDocument(
      appwriteConfig.databaseId,
      appwriteConfig.counterCollectionId, // Collection to store the counter
      'complaintCounter' // Fixed ID for counter document
    )

    const currentCounter = counterDoc.currentCounter;
    const newCounter = currentCounter + 1;

    await databases.updateDocument(
      appwriteConfig.databaseId,
      appwriteConfig.counterCollectionId,
      "complaintCounter",
...
export const getLastComplaint = async (userId) => {
  try {
    const complaints = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.complaintsCollectionId,
      [
        Query.equal('consumer_id', userId),
        Query.orderDesc('createdAt'),
        Query.limit(1), // Fetch the most recent complaint
      ]
    );

    return complaints.documents.length > 0 ? complaints.documents[0] : null;
  } catch (error) {
